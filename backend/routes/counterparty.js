...
/**
 * @route   GET /api/counterparty/invite/:token
 * @desc    Get invitation details by token (for accept invite page)
 * @access  Public
 */
router.get('/invite/:token', async (req, res) => {
  try {
    ...
    console.log(`📧 Fetching invitation details for token: ${token.substring(0, 10)}...`);
    ...
    if (invitation.connectionStatus !== 'PENDING') {
    ...
    console.log(`✅ Found valid invitation from ${invitation.ourCustomerName}`);
    ...
    res.json({
      ...
      invitation: {
        ...
        isExpired: isExpired,
        relationshipType: invitation.relationshipType || 'customer'
      }
    });
